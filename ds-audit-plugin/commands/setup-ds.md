...
## Purpose
One-time guided setup for the DS Audit Plugin. Collects DS-specific configuration from the user and saves it to config.json in the plugin root. All other commands depend on this file.
...
Then write config.json to the plugin root folder with the following structure:
...
config.json has been saved to the plugin folder.
...
- If the config file already exists, ask: Update existing config or start fresh?
